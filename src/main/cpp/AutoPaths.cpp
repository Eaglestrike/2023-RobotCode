...
void AutoPaths::setPath(Path path)
{
    ...
    case ROTATE_ONLY_ROTATE:{
        double forward = isBlue_? 1.0 : -1.0;
        double yaw = forward * 90.0 + 89.9;
        swervePoints_.push_back(SwervePose(0.0, 0.0, yaw));
    }
    case NOTHING:{break;}
...
void AutoPaths::periodic()
{
    ...
    if (path_ != DRIVE_BACK_DUMB &&
        path_ != NOTHING && 
        path_ != WAIT_5_SECONDS && 
        path_ != TAXI_DOCK_DUMB && 
        path_ != NO_TAXI_DOCK_DUMB)
    {
        SwervePose pose;
        for (size_t i = pointNum_; i < swervePoints_.size(); ++i){
                    ...
                    (!curveSecondStageGenerated_ || !yawStageGenerated_)))
            {
                ...
                else if ((path_ == SECOND_CUBE_DOCK || path_ == SECOND_CUBE_GRAB)){
                    if (pointNum_ == 0){
                        if (!pathGenerated_){
                            double setX = swervePoints_[i].x;
                            generateXTraj(currPose.x, setX, swerveDrive_->getXYVel().getX());
                        ...
                        if ((!curveSecondStageGenerated_) && curveReady && isStationary(yProfile)){
                            double setY = swervePoints_[i].y;
                            generateYTraj(currPose.y, setY, swerveDrive_->getXYVel().getY());
                            curveSecondStageGenerated_ = true;
                ...
                else if(path_ == ROTATE_ONLY_ROTATE){
                    if(!pathGenerated_){
                        double forward = isBlue_? 1.0 : -1.0; //Forward direction
                        double yaw = forward * 90.0 + 89.9;
                        yawTraj_.generateTrajectory(currPose.yaw, yaw, 0); // 
            ...
            else if ((path_ == FIRST_CUBE_DOCK && pointNum_ == 1))
            {
                ...
                Pose1D xProfile = {currPose.x, 0.0, 0.0};
                Pose1D yProfile = {currPose.y, 0.0, 0.0};
                Pose1D yawProfile = yawTraj_.getProfile();
                pose = SwerveFromPose1D(xProfile, yProfile, yawProfile);
                if (isStationary(pose)){
                    pointOver = true;
            ...
            if (!pointOver){
                ...
                else if (nextPointReady_ && i != swervePoints_.size() - 1){ //Coming back
                    nextPointReady_ = false;
                    ++pointNum_;
                    startTimer();
                    pathGenerated_ = false;
        ...
        if (!isZero(pose)){
            if (timer_.GetFPGATimestamp().value() - autoStartTime_ > 14.9 &&
                (path_ == SECOND_CUBE_DOCK || path_ == FIRST_CUBE_DOCK || path_ == AUTO_DOCK)) //If ended and docked
            {
    ...
    case FIRST_CUBE_HIGH:
    {
        if (pointNum_ == 0){
            ...
            if (firstCubeArmSafety_){
                ...
                if (timer_.GetFPGATimestamp().value() - startTime_ > 0.4){ //Start moving arm before reaching placing location, was 0.2
                    cubeIntaking_ = false;
                    forward_ = true;
                    armPosition_ = TwoJointArmProfiles::CUBE_HIGH;
                }
            ...
            if((armPlacing_ && (timer_.GetFPGATimestamp().value() - armStart_ > 1.1)) || // Score some time after arm extends
                (arm_->getPosition() == TwoJointArmProfiles::CUBE_HIGH && arm_->getState() == TwoJointArm::HOLDING_POS && pointOver)){ //Score when reached target
                wheelSpeed_ = ClawConstants::OUTAKING_SPEED * 2.0; // Increase speed
                if (!placingTimerStarted_){
                    placingStartTime_ = timer_.GetFPGATimestamp().value();
                    placingTimerStarted_ = true;
                }
                if ((timer_.GetFPGATimestamp().value() - placingStartTime_ > 0.2) && (arm_->getPosition() == TwoJointArmProfiles::CUBE_HIGH && arm_->getState() == TwoJointArm::HOLDING_POS)){ // Place for 0.2 seconds
                    nextPointReady_ = true;
                    placingTimerStarted_ = false;
                }
            }
    ...
    case SECOND_CUBE_MID:
    {
        ...
        else
        {
            ...
            else if (timer_.GetFPGATimestamp().value() - startTime_ > 0.4 && armReady){
                cubeIntaking_ = false;
                forward_ = true;
                armPosition_ = TwoJointArmProfiles::CUBE_MID;
            }
    ...
    case TAXI_DOCK_DUMB:
    {
        ...
        if(timer_.Get().value() > 1.5){
            ...
            if (!comingDownChargingStation_){ //Drive over charge to come down
                ...
                double forwardVel = (sin(sin(time - 3.0 - 1.5) + 2.3) + cos(time - 0.9 - 1.5))/2.0; //Goofy desmos graph
                vel *= forwardVel;
    ...
    case NO_TAXI_DOCK_DUMB:
    {
        ...
        if(time > 0.5){
            ...
            if (!onCharge_){ //Drive over charge to come down
                double vel = forward * SwerveConstants::SENDING_IT_FAST_SPEED;
                double forwardVel = (sin(sin(time - 3.0 + 1.0) + 2.3) + cos(time - 0.9 + 1.0))/2.0; //Goofy desmos graph
                vel *= forwardVel;
                swerveDrive_->drive({vel, 0}, 0);
            }
            else{
...
Point AutoPaths::initPos(){
    ...
    case ROTATE_ONLY_ROTATE:{return {0.0, 0.0};}

...
void TwoJointArmProfiles::readProfiles()
{
	...
	for(int i = STOWED; i <= CONE_INTAKE; ++i)
	{
		...
		{
            //if path isn't 03 or 30
			if(! ( (i == STOWED && j == MID) || (i == MID && j == STOWED) ) )
			{
				continue;
			}
...
{
    map<pair<Positions, Positions>, map<double, pair<tuple<double, double, double>, tuple<double, double, double>>>>::iterator it;

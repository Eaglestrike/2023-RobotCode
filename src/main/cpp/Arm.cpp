...
void Arm::Periodic() {
    ...
    frc::SmartDashboard::PutNumber("Base Voltage unclamped", ffu_volts[0]);
    frc::SmartDashboard::PutNumber("Top Voltage unclamped", ffu_volts[1]);
    frc::SmartDashboard::PutNumber("Base Angle", baseReading);
    frc::SmartDashboard::PutNumber("Top Angle", topReading);
    frc::SmartDashboard::PutNumber("Base Target", ang1);
    frc::SmartDashboard::PutNumber("Top Target", ang2);
    
    baseVoltage = std::clamp(ffu_volts[0], -m_maxVolts, m_maxVolts);
    m_baseMotor.SetVoltage(units::voltage::volt_t(baseVoltage));

    topVoltage = std::clamp(ffu_volts[1], -m_maxVolts, m_maxVolts);
